...
def df_csv_to_PDF(df_or_csv, PDF_template, output_filename, colkey=None, checkbox_fields=None, output_loc=None):
    ...
    Args:
        df_or_csv (dataframe or csv path): input data, must be a pandas dataframe or full path to csv file.
        PDF_template (pdf path): PDF to fill with input data, full path to PDF file.
    ...
    df = convert_csv_to_df(df_or_csv) if isinstance(df_or_csv, str) else df_or_csv
